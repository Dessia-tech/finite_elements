...
class MagnetLoad(DessiaObject):
    ...
    def __init__(self, elements: List[vmmesh.TriangularElement],
                 non_contour_nodes: List[vm.Point2D], magnetization_vector: vm.Vector2D):
        self.elements = elements
...
class FiniteElementAnalysis(DessiaObject):
    ...
    def __init__(self, mesh: vmmesh.Mesh, element_loads: List[ConstantLoad], node_loads: List[SingleNodeLoad],
                 magnet_loads: List[MagnetLoad], continuity_conditions: List[ContinuityCondition]):
        self.mesh = mesh
    ...
    def create_matrix(self):
        ...
        for i, condition in enumerate(self.continuity_conditions):
            ...
            row_ind.extend((len(self.mesh.nodes) + len(self.node_loads) + i, index1,
                           len(self.mesh.nodes) + len(self.node_loads) + i, index2))
            col_ind.extend((index1, len(self.mesh.nodes) + len(self.node_loads) + i,
                           index2, len(self.mesh.nodes) + len(self.node_loads) + i))
    ...
    def create_source_matrix(self):
        ...
        for load in self.element_loads:
            for element in load.elements:
                ...
                double_integral_N1_dS = det_jacobien * \
                    (a1 + 0.5 * b1 * x2 + 0.5 * c1 * y2 + 0.5 * b1 * x3 + 0.5 * c1 * y3)
                double_integral_N2_dS = det_jacobien * \
                    (a2 + 0.5 * b2 * x2 + 0.5 * c2 * y2 + 0.5 * b2 * x3 + 0.5 * c2 * y3)
                double_integral_N3_dS = det_jacobien * \
                    (a3 + 0.5 * b3 * x2 + 0.5 * c3 * y2 + 0.5 * b3 * x3 + 0.5 * c3 * y3)
